...
Crafty.extend({
     ...
     */
    init: function (w, h, stage_elem, mode) {
        ...
        Crafty.viewport.init(w, h, stage_elem, mode);
...
Crafty.extend({
    domLayer: {
        ...
        _render: function () {
            ...
            for (; i < k; ++i) {
                changed[i]._element.style.display = 'none';
            }
            ...
            //loop over all DOM elements needing updating
            for (; i < k; ++i) {
                changed[i]._element.style.display = 'block';
            }
    ...
    draw: function () {
        ...
        if (Crafty.support.css3dtransform && !this.avoidCss3dTransforms) {
            if(Crafty.mobile) {
                ...
                trans.push("translate(" + (~~this._x) + "px," + (~~this._y) + "px)");
            }
        } else {
        ...
        if (this._scaleX) {
            trans.push("scaleX("+this._scaleX+")");
        }

        if (this._scaleY) {
            trans.push("scaleY("+this._scaleY+")");
        }

        if (this._skewX) {
            trans.push("skewX("+this._skewX+"deg)");
        }

        if (this._skewY) {
            trans.push("skewY("+this._skewY+"deg)");
        }
...
Crafty.c("Text", {
    ...
    init: function () {
        ...
        this._overflow = this.defaultOverflow;
        this._wordwrap = this.wordwrap;
...
Crafty.extend({
    ...
    viewport: {
         ...
         */
        init: function (w, h, stage_elem, resolutionMode) {  
            ...
            this._resolutionMode = resolutionMode || 2;
            ...
            // Crafty.addEvent(this, window, "orientationchange", Crafty.viewport.orientationChange);
            ...
            elem.imageRendering = "-webkit-optimize-contrast";
            ...
            if (Crafty.mobile) {
                ...
                var i=0,flag=false,content='width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no';
         ...
         */
        setDesignResolutionSize: function(width, height) {
            var elem = Crafty.stage.elem.style,
            ...
            result = this.getResolutionPolicy();
            if(result.scale && result.scale.length === 2){
                scaleX = result.scale[0];
                scaleY = result.scale[1];
            }
           
            if ( this._resolutionMode == 3) {
                this._width = innerWidth / scaleX;
                this._height = innerHeight / scaleY;
            }
            
            marginScale = this._resolutionMode == 2 ? 0.5: 1;
            elem.width = this._width + "px";
            elem.height = this._height + "px";
            elem.position = "relative";
            elem.transform = "scale(" + scaleX + "," + scaleY + ")";
            elem.webkitTransform = "scale(" + scaleX + "," + scaleY + ")";
            elem.transformOrigin = "top left";
            elem.webkitTransformOrigin = "top left";
            marginLeft = (innerWidth - (this._width * scaleX)) * marginScale;
            marginTop = (innerHeight - (this._height * scaleY)) * marginScale;
            elem.marginLeft = marginLeft + "px";
            elem.marginTop = marginTop + "px";
        },
        ...
        getResolutionPolicy: function() {
            ...
            if (this._resolutionMode == 0) {
                scaleX = innerWidth / this._width;
                scaleY = innerHeight / this._height;
                result.scale = [scaleX, scaleY];
            } else if (this._resolutionMode == 1) {
                scaleX = innerWidth / this._width;
                scaleY = innerHeight / this._height;
                scaleX < scaleY ? (scaleX = scaleY) : (scaleY = scaleX);
                result.scale = [scaleX, scaleY];
            } else if (this._resolutionMode == 2) {
                scaleX = innerWidth / this._width;
                scaleY = innerHeight / this._height;
                scaleX < scaleY ? (scaleY = scaleX) : (scaleX = scaleY);
                result.scale = [scaleX, scaleY];
            } else if (this._resolutionMode == 3) {
                scaleY = innerHeight / this._height;
                scaleX = scaleY;
                result.scale = [scaleX, scaleY];
            } else if (this._resolutionMode == 4) {
                scaleX = innerWidth / this._width;
                scaleY = scaleX;
                result.scale = [scaleX, scaleY];
            }
...
Crafty.extend({
    ...
    audio: {
         ...
         * 
         */
        getChannelObj:function(id){
              for(var i in this.channels){
                if(this.channels[i].id = id){
                    return this.channels[i].obj;
                }
            }
            return null;
        },
...
Crafty.c("2D", {
    ...
    _2D_property_definitions: {
        ...
        _scaleX: {enumerable:false},

        scaleY: {
            set: function (v) {
                this._attr('_scaleY', v);
            },
            get: function () {
                return this._scaleY;
            },
     ...
     * #._cascade
     * @comp 2D
     * @sign public void ._cascade(e)2s
    ...
    _cascade: function (e) {
        ...
        } else if ("_visible" in e) {
            for (; i < l; ++i) {
                obj = children[i];
                if ('visible' in obj) obj._attr('visible', e["_visible"]);
            }
        } else if ("_alpha" in e) {
            //if ('alpha' in obj) obj._attr('alpha', e["_alpha"]);
        } else if ("_scaleX" in e) {
            //if ('scaleX' in obj) obj._attr('scaleX', e["_scaleX"]);
        } else if ("_scaleY" in e) {
            //if ('scaleY' in obj) obj._attr('scaleY', e["_scaleY"]);
        } else {
    ...
    attach: function () {
        ...
        for (; i < l; ++i) {
            ...
            this._element.appendChild(obj._element);
        }
    ...
    _attr: function (name, value) {
        ...
        if (name === '_rotation') {
            ...
            this.trigger("Visible", {'_visible' : value});
        }
